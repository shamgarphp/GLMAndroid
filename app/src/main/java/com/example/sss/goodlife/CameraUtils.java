...
public class CameraUtils {
    ...
    public static File getOutputMediaFile(int type) {
        ...
        File mediaStorageDir = new File(
                ...
                UploadVbsPhotos.GALLERY_DIRECTORY_NAME);

        // Create the storage directory if it does not exist
        if (!mediaStorageDir.exists()) {
            if (!mediaStorageDir.mkdirs()) {
                Log.e(UploadVbsPhotos.GALLERY_DIRECTORY_NAME, "Oops! Failed create "
                        + UploadVbsPhotos.GALLERY_DIRECTORY_NAME + " directory");
                return null;
            }
        }
        ...
        File mediaFile;
        if (type == UploadVbsPhotos.MEDIA_TYPE_IMAGE) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "IMG_" + timeStamp + "." + UploadVbsPhotos.IMAGE_EXTENSION);
        } else if (type == UploadVbsPhotos.MEDIA_TYPE_VIDEO) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "VID_" + timeStamp + "." + UploadVbsPhotos.VIDEO_EXTENSION);
        } else {
            return null;
        }
